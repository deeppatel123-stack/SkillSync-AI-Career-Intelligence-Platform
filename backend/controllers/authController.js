...
async function register(req, res) {
  try {
    ...
    await sendEmail({
      to: user.email, subject: 'Welcome to SkillSync',
    ...
    res.status(201).json({ success: true, message: 'Registration successful', user: user.toPublicJSON() });
  } catch (error) {
...
async function login(req, res) {
  try {
    ...
    if (user.role === 'superadmin') return res.status(403).json({ success: false, message: 'Please use the admin login page' });

    loginSession(req, user);
    res.json({ success: true, message: 'Login successful', user: user.toPublicJSON() });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
}
...
async function adminRegister(req, res) {
  try {
    ...
    const hashed = await bcrypt.hash(password, 10);
    await User.create({ name, email: email.toLowerCase(), password: hashed, role: 'superadmin', organization: organization || 'SkillSync', phone: phone || '' });
...
async function adminLogin(req, res) {
  try {
    ...
    loginSession(req, user);
    res.json({ success: true, message: 'Admin login successful', user: user.toPublicJSON() });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
}
...
module.exports = { register, login, adminRegister, adminLogin, logout, getMe };
